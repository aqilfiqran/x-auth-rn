 ...
 */
export function getGeneralApiProblem(problem: string, status: number): GeneralApiProblem | null {
  switch (problem) {
...
export function getProblemFromError(error: AxiosError) {
  ...
  if (!error.code) {
    return getProblemFromStatus(error.response ? error.response.status : undefined)
  }
